...
def on_ticks(ws, ticks):
    global pivot_price, moving_pivot_on, limits, away_from_circuit, lower_circuit, upper_circuit,last_min,last_sec
    ...
    if(((upper_circuit-away_from_circuit)>ticks[0]['last_price']) 
       and ((lower_circuit+away_from_circuit)<ticks[0]['last_price'])):
        if (last_sec!=this_sec and this_sec%5==0):
            last_min = this_min
            last_sec = this_sec
            print('in the thread : ')
